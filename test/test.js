...
var iterHaversin = require( './../lib' );


// VARIABLES //

var rand = randu( 0.0, 6.28 );
...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterHaversin, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterHaversin( uniform( 0.0, 6.28 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterHaversin( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the half-value versed sine for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
	...
	N = 101;
	rand = uniform( 0.0, 6.28, {
		'iter': N
	});
	it = iterHaversin( rand );
	t.equal( it.next.length, 0, 'has zero arity' );
	...
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = haversin( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the half-value versed sine for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': haversin( values[ 0 ] ),
			'done': false
		},
		{
			'value': haversin( values[ 1 ] ),
			'done': false
		},
		{
			'value': haversin( values[ 2 ] ),
			'done': false
		},
		{
			'value': haversin( values[ 3 ] ),
			'done': true
		}
	];
